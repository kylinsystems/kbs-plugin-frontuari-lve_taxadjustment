...
import org.frontuari.info.LVE_TaxAdjustmentPluginFeatures;
 ...
 *
 */
public class LVE_TaxAdjustmentModelFactory  implements IModelFactory {
	
	private final static CLogger log = CLogger.getCLogger(LVE_TaxAdjustmentModelFactory.class);
	...
	public Class<?> getClass(String tableName) {
		...
		if (clazz == null) {
			...
			Arrays.sort(LVE_TaxAdjustmentPluginFeatures.entityType);
			if (Arrays.binarySearch(LVE_TaxAdjustmentPluginFeatures.entityType, entityType)< 0)
				return null;
			...
			try {
				...
				try {
					...
					if (log.isLoggable(Level.WARNING))
						log.warning(String.format("Plugin: %s -> Class not found for table: %s", LVE_TaxAdjustmentPluginFeatures.id, tableName));
				}
			}
		}
	...
	public PO getPO(String tableName, int Record_ID, String trxName) {
		...
		try {
			...
			if (log.isLoggable(Level.WARNING))
				log.warning(String.format("Plugin: %s -> Class can not be instantiated for table: %s", LVE_TaxAdjustmentPluginFeatures.id, tableName));
		}
